...
for (p in seq(nprot)) { # loop over proteins
  ...
  # debug: odd: for mutants, all sij = dij - lij < 0, don't use for now
  # wt_0$enm$p_null <- p_null_chain(wt_0, param$enm)
  ...
  for (m in seq(nmut)) {
    mutant <- get_mutant(wt_t, wt_0, ideal = wt_0, param)
    mut <- mutant$mut
