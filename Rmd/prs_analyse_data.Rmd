...
dfej_long <- dfej %>% 
  filter(mutation > 0) %>% 
  pivot_longer(
    cols = c("delta_v_stress","delta_v_min", "delta_g_entropy"),
    names_to = "response",
...
p1 <- inner_join(dfej, de2) %>% 
  ggplot(aes(delta_v_stress - delta_v_min, de2)) +
  geom_point() +
  geom_abline() +
  theme_minimal()
...
p2 <- inner_join(dfej, de2) %>% 
  group_by(j) %>% 
  summarise(delta_v_stress = mean(delta_v_stress),
            delta_v_min = mean(delta_v_min),
            de2 = mean(de2)) %>% 
  ggplot(aes(delta_v_stress - delta_v_min, de2)) +
  geom_point() +
  geom_abline() +
