...
dfej_long <- dfej %>%
  filter(mutation > 0) %>%
  pivot_longer(
    cols = c("dvs","dvm", "dg_entropy"),
    names_to = "response",
    values_to = "calculate_dfj"
  )
```
...
p_site <- dfej_long %>%
  ...
  mutate(calculate_dfj = mean(calculate_dfj)) %>%
  ggplot(aes(j, calculate_dfj)) +
  geom_line() +
...
p_mlmsj <- dfej_long %>%
  group_by(j, msfj, mlmsj, response) %>%
  mutate(calculate_dfj = mean(calculate_dfj)) %>%
  ggplot(aes(mlmsj, calculate_dfj)) +
  geom_point(size = .5) +
  geom_smooth() +
  facet_wrap(~ response, scales = "free_y", ncol = 1) +
  theme_minimal()
...
p1 <- dfej_long %>%
  filter(mutation %in% c(1, 2)) %>%
  mutate(mutation = factor(mutation)) %>%
  ggplot(aes(j, calculate_dfj, color = mutation, group = mutation)) +
  geom_line() +
...
p2 <- dfej_long %>%
  filter(mutation %in% c(1, 2)) %>%
  mutate(mutation = factor(mutation)) %>%
  select(j, mutation, response, calculate_dfj) %>%
  pivot_wider(names_from = mutation,
              names_prefix = "mutation_",
              values_from = calculate_dfj) %>%
  ggplot(aes(mutation_1, mutation_2)) +
  geom_point(size = .5) +
  geom_abline() +
  theme_cowplot() +
