...
#' Compare two protein nsembles mode by mode (mode-dependent profiles)
#'
#' Given two proteins, compare their conformational ensembles (fluctuation patterns),
#' the proteins'  \code{cmat}, and normal modes (Principal components) are assumed known.
#'
...
#' @rdname mode_ensemble_profile
#'
#' @details `calculate_dmsfn` returns mode-dependent profile of changes of mean-square fluctuations \eqn{\delta \sigma_n^2}
#'
#' @export
#'
calculate_dmsfn <- function(wt, mut) {
  stopifnot(wt$node$pdb_site == mut$node$pdb_site) # no indels
...
#' @rdname mode_ensemble_profile
#'
#' @details `calculate_dhn` returns mode-dependent profile of entropy differences \eqn{\delta H_n}
#'
#' @export
#'
calculate_dhn <- function(wt, mut) {
  stopifnot(wt$node$pdb_site == mut$node$pdb_site) # no indels
...
#' @rdname mode_ensemble_profile
#'
#' @details `calculate_rwsipn` returns mode-dependent profile of rwsip similarity
#'
#' @export
#'
calculate_rwsipn <- function(wt, mut) {
  stopifnot(wt$node$pdb_site == mut$node$pdb_site) # no indels
...
#' @rdname mode_ensemble_profile
#'
#' @details `calculate_nHn` returns mode-dependent profile of mode conservation  measure \eqn{nH_n}
#'
#' @export
#'
calculate_nhn <- function(wt, mut) {
  stopifnot(wt$node$pdb_site == mut$node$pdb_site) # no indels
  overlap <- crossprod(get_umat(wt), get_umat(mut))
  pmat <- overlap^2
