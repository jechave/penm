...
# Calculo de las matrices de PRS.
### Funcion de la matrix de covarianza o de la inversa del hessiano obtenida por ANM y del nº de fuerzas al azar aplicadas

prs_nacho <- function(covar_mat, n_randforces){
  a <- -0.5
  b <- 0.5
...
prs_nacho_fast <- function(cmat, normalise = F) {
  nsites <- nrow(cmat) / 3
