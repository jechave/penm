...
%%% ---------------------------------------------------------------------
%%% Annotations added 2026-08-20. The body text below is unmodified.
%%% Every addition is wrapped in \claude{} (theory) or \claudecode{}
%%% (implementation), so all of it can be removed with
%%%     grep -v '\\claude' , or hidden by redefining both to {}:
%%%     \renewcommand{\claude}[1]{}  \renewcommand{\claudecode}[1]{}
%%% ---------------------------------------------------------------------
\usepackage[dvipsnames]{xcolor}
\newcommand{\claude}[1]{%
  \par\smallskip\noindent
  \textcolor{TealBlue}{\sffamily\small\textbf{[T]}~#1}%
  \par\smallskip}
\newcommand{\claudecode}[1]{%
  \par\smallskip\noindent
  \textcolor{Plum}{\sffamily\small\textbf{[C]}~#1}%
  \par\smallskip}
...
\claude{\textbf{About this copy.} The body text is Julian Echave's 2017
document, unmodified. Annotations added 2026-08-20 are marked
\textbf{[T]} (theory: corrections, settled questions, open ones) and
\textbf{[C]} (correspondence with the \texttt{penm} package). They are
suggestions for the author to accept or reject, not edits. To remove them all,
redefine \texttt{\textbackslash claude} and \texttt{\textbackslash claudecode}
to \texttt{\{\}} in the preamble.}
...
\claude{\textbf{The sign is settled: the second form is right.} Differentiating
$V=\frac{1}{2}\kij(\dij-\lij)^2$ twice gives, for a central pair potential,
$\kmat_{ij} = -[V''\,\eij\eij^T + (V'/\dij)(\mathbf{I}-\eij\eij^T)]$ with
$V''=\kij$ and $V'=\kij(\dij-\lij)$; written with the bracket
$(\eij\eij^T-\mathbf{I})$ as above, the coefficient is
$(\lij/\dij-1)$. Checked numerically against a central-difference Hessian on
four random \emph{frustrated} 5-node networks ($\lij\neq\dij$): this form agrees
to $\sim10^{-4}$ (finite-difference noise), the other is off by $O(1)$ (2--7).
The two boxes above are therefore not two candidate results but one result
written two ways --- each is correct when paired with its own bracket ordering.}

\claude{\textbf{The trace argument drops a $\dij$.} $s_{ij}=\lij-\dij$ is a
length, so $1-2s_{ij}$ subtracts a length from a pure number and the threshold
$s_{ij}>1/2$ has no invariant meaning --- it moves if \AA{} is replaced by nm.
The coefficient of the transverse term is the dimensionless $\lij/\dij-1$, so
with $Tr(\eij\eij^T)=1$ and $Tr(\eij\eij^T-\mathbf{I})=-2$,
$Tr(\kmat_{ij}) = -\kij(3-2\lij/\dij)$, changing sign at $\lij/\dij=3/2$: a
spring compressed to two-thirds of its rest length. Severe, but not absurd.}

\claude{\textbf{What a negative effective $\kij$ means physically: buckling.}
A spring with $\lij>\dij$ is compressed. Displacing $j$ transversally at fixed
$\dij$ lengthens the gap along no direction the spring resists, while relieving
compression, so the stored energy is \emph{released} by the sideways motion ---
exactly a strut past its critical axial load. This is not unphysical in itself:
only the full $\kmat$ need be positive semi-definite at a minimum, and a single
block with $Tr>0$ can be stabilised by the neighbouring springs.
\textbf{But it constrains the choice of $\{\lij\}$:} if the frustration is severe enough
that $\kmat$ acquires a negative eigenvalue, then $\rvectorEq$ satisfies the
stationarity condition (\ref{eq:VminCondition3}) while being a \emph{saddle},
not a minimum. So the admissible $\{\lij\}$ are not all solutions of
(\ref{eq:VminCondition3}), but those intersected with $\kmat$ positive semi-definite --- a
bounded region, not an affine subspace. Worth stating where the
underdetermination of $\{\lij\}$ is discussed below.}

\claudecode{\texttt{calculate\_enm\_kmat()} implements
$\texttt{gij} = \lij/\dij - 1$ with the bracket $(\eij\eij^T-\mathbf{I})$ ---
i.e.\ the second form, the one verified above. So the implementation is
consistent with the correct Hessian, and the \texttt{\#WARNING} the package
carries about this sign can be retired. It is never exercised, however:
\texttt{set\_enm()} has \texttt{stopifnot(!frustrated)}, so only the
$\texttt{gij}=0$ branch ever runs.}
...
\claude{\textbf{Two riders on ``valid''.} (i) (\ref{eq:VminCondition3}) is a
\emph{stationarity} condition, so it makes $\rvectorEq$ a critical point, not
necessarily a minimum. As noted at the Hessian above, severe frustration can
drive an eigenvalue of $\kmat$ negative, at which point $\rvectorEq$ is a saddle
and the model is not a valid ENM of that structure. The admissible set is the
solutions of (\ref{eq:VminCondition3}) intersected with $\kmat$ positive
semi-definite --- bounded, and no longer an affine subspace.
(ii) Two such models share $\rvectorEq$ but \emph{not} their dynamics: $\kmat$
depends on $\{\lij\}$ through the transverse term, so different admissible
$\{\lij\}$ give different normal modes and fluctuations. ``Valid ENM model'' is
therefore a statement about structure only. This is what makes frustration
observable in principle: it cannot be seen in $\rvectorEq$, only in motion.}

\claude{\textbf{Counting.} Read as equations for $\{\lij\}$ at fixed
$\rvectorEq$, (\ref{eq:VminCondition3}) is \emph{linear} in $\lij$, of rank
generically $3N-6$. So the solution set is an affine subspace of dimension
$M-(3N-6)$, where $M=N\overline{CN}/2$ is the number of springs. Its being
linear is what makes the fitting strategies of the appendix sensible.}
...
\claudecode{\textbf{Status in the package.} This argument --- mutation generates
frustration, so a non-frustrated model is insufficient --- is the one place the
theory and the implementation are in open tension. \texttt{penm} builds every
wild type with $\lij=\dijEq$ and blocks \texttt{frustrated = TRUE} outright
(\texttt{stopifnot(!frustrated)}). Mutants are frustrated by construction, as
this paragraph says, but their $\kmat$ is built with the transverse term
switched off, so the frustration enters the energy and never the dynamics. A
2022 note in the package log records the same doubt from the other side
(``\texttt{mut\_graph} assumes frustrated = T?~For now use frustrated = F
everywhere, until I check it thoroughly'') and no later entry records the check.}
...
\claude{\textbf{Why dropping $\delta\kmat$ is consistent, and what it costs.}
In the Hamiltonian the neglected term enters as
$(\rvector-\rvectorEq)^T\,\delta\kmat\,(\rvector-\rvectorEq)$. With
$\delta\rvector = O(\dlij)$ and $\delta\kmat = O(\dlij)$, that is third order in
small quantities against the second-order terms retained, so LFENM is a
controlled truncation \emph{of the energy}. Note this does not make
$\delta\kmat$ itself higher order --- it is $O(\dlij)$, the same order as
$\delta\rvector$. Consequently LFENM is silent, not wrong, about dynamics: mode
shapes and $\delta(TS)$ are $O(\dlij)$ quantities that the truncation sets to
zero. That is the gap SC-LFENM fills, and it is worth stating explicitly when
the \texttt{todo} items above are written up.}

\claudecode{\textbf{As implemented.} Mutation perturbs $\lij$ only
($\delta\vijMin=0$, $\delta\kij=0$), draws $\dlij$ i.i.d.\ with
$\langle\dlij\rangle=0$ and variance $\sigma^2$ over the springs of the mutated
site, then relaxes by $\delta\rvector=\cmat\mathbf{f}$ using the wild-type
compliance. \texttt{lfenm} stops there; \texttt{sclfenm} additionally rebuilds
$\kmat$ and re-diagonalises. Both compute $\delta\rvector$ from the wild type,
so the two models give \emph{identical} mutant structures and differ only in the
Hessian --- hence all structure-based comparisons agree between them and all
motion-based ones vanish identically under \texttt{lfenm}.}
...
\claude{Two notes on this strategy. (i) For unordered pairs with repetition over
20 amino acids the count is $20\cdot21/2 = 210$, not 220. (ii) It sits in
tension with the mutational model of \S4, which has no amino-acid alphabet at
all --- mutations there are random draws of $\dlij$, so a site has no identity
that $\lij$ could depend on. The two would need reconciling before both are
