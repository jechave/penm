...
df <- tibble(prot = c("wt", "mutff", "mutft", "muttf", "muttt"),
  ...
  mutate(mut_energy = pmap(list(wt, site, mutation, update_enm, add_frust), get_mut_energy)) %>% 
  unnest(mut_energy) %>% 
  arrange(mutation, update_enm, add_frust) %>% 
  select(-prot, -wt, -mutation) 
...
update_enm <- T
add_frust <- F
mut <- get_mut(wt, mutated_site, mutation = 1, dl_sigma = 1, update_enm, add_frust)

# set msf data.frame for plotting
site <- prot_site(wt)
wt_msf <- prot_msf(wt)
mut_msf <- prot_msf(mut)
...
# set msf data.frame for plotting
mode <- prot_mode(wt)
wt_msf <- 1 / prot_evalue(wt)
mut_msf <- 1 / prot_evalue(mut)
